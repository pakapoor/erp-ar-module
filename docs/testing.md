...
| `outbox_publisher` | `erp_outbox_publisher` | Up | PostgreSQL outbox â†' SQS relay |
| `delivery_worker` | `erp_delivery_worker` | Up | SQS â†' idempotent adapter consumer |
...
| API4 foreign-currency loss/partial | Two lower-rate receipts move APPROVED â†' PARTIALLY_PAID â†' PAID, debit both realized INR losses and leave zero AR |
...
| CFO retry | DEAD â†' PENDING â†' DELIVERED; invoice becomes SENT without repeating accounting |
...
foreignâ†'INR pairs, and stores tenant-approved immutable rows. The walkthrough
...
has seven approved foreignâ†'INR rows. A weekend request legitimately uses the
