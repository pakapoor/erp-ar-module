...
|------------------|-----|-------------|------|--------|--------------|
| Industrial Pump  |  2  | INR 50,000  | 18%  | MH     | INR 1,18,000 |
| Safety Valves    | 10  | INR 5,000   | 12%  | KA     | INR 56,000   |

Subtotal:  INR 1,50,000
Tax:       INR 24,000
Total:     INR 1,74,000
Currency:  INR
```
...
Subtotal:        INR 1,50,000
Tax:             INR 24,000
Invoice Total:   INR 1,74,000
Paid:            INR 1,00,000
Credit Memos:   -INR 10,000
Outstanding:     INR 64,000

Payment History:
P001  Jan 20  INR 1,00,000  RTGS  UTR:HDFC2024012000123  Applied
CM001 Jan 25  INR 10,000    Credit Memo                   Applied
...
**Approval Thresholds:**
- Invoice < INR 1,00,000 -- Accounts Officer can approve
- Invoice < INR 10,00,000 -- Finance Manager can approve
- Invoice > INR 10,00,000 -- CFO must approve
...
Step 2: Rahul submits for approval
        Amount INR 1,74,000 < INR 10,00,000
...
Debit:  1200 AR           INR 1,74,000  -- Tata Steel owes Reliance
Credit: 3100 Revenue      INR 1,50,000  -- Reliance earned it
Credit: 2200 Tax Payable  INR   24,000  -- collected for the government
...
Invoice #1001  INR 1,74,000  due Jan 31  (oldest)
Invoice #1002  INR 80,000    due Jan 31
Invoice #1003  INR 46,000    due Jan 31

Payment #P001 received: INR 3,00,000 (RTGS)
UTR: HDFC2024013100123
Mode: Auto (FIFO)

Allocation:
+-- Invoice #1001 INR 1,74,000 -- Fully Paid
+-- Invoice #1002 INR 80,000   -- Fully Paid
+-- Invoice #1003 INR 46,000   -- Fully Paid

GL Entry:
Debit:  1100 Cash    INR 3,00,000  (ref: P001)
Credit: 1200 AR      INR 3,00,000  (ref: P001)

---

Example 2 -- Partial Payment (Auto FIFO):
Payment #P002 received: INR 2,00,000
Allocation:
+-- Invoice #1001 INR 1,74,000 -- Fully Paid
+-- Invoice #1002 INR 26,000   -- Partially Paid (INR 54,000 remaining)
Invoice #1003 -- Untouched

GL Entry:
Debit:  1100 Cash    INR 2,00,000  (ref: P002)
Credit: 1200 AR      INR 2,00,000  (ref: P002)
...
Payment #P003 received: INR 2,00,000
Mode: Manual (remittance advice from Tata Steel)
Allocation:
+-- Invoice #1001 INR 1,00,000
+-- Invoice #1002 INR 60,000
+-- Invoice #1003 INR 40,000

Invoice Status:
#1001 -- Partially Paid (INR 74,000 remaining)
#1002 -- Partially Paid (INR 20,000 remaining)
#1003 -- Partially Paid (INR 6,000 remaining)

---

Example 4 -- Overpayment:
Payment #P004 received: INR 4,00,000
Total outstanding:      INR 3,00,000
Overpayment:            INR 1,00,000

GL Entry:
Debit:  1100 Cash             INR 4,00,000
Credit: 1200 AR               INR 3,00,000  -- clears all invoices
Credit: 2100 Customer Credit  INR 1,00,000  -- liability, we owe them
...
Invoice #2001: $1,000 (rate Jan 1 = 83) = INR 83,000 base

Payment 1: Feb 1  $600  rate=80
Debit:  Cash         INR 48,000
Debit:  FX Loss      INR 1,800
Credit: AR           INR 49,800

Payment 2: Apr 1  $400  rate=95
Debit:  Cash         INR 38,000
Credit: AR           INR 33,200
Credit: FX Gain      INR 4,800

Net FX Gain: INR 3,000
```
...
Invoice #1001: INR 1,74,000 (Sent to Tata Steel)
Tata Steel returns 2 damaged Safety Valves

Credit Memo #CM001:
Against Invoice: #1001
Reason: RETURN
Lines: Safety Valves -- 2 @ INR 5,000 = INR 10,000
Approved by: Priya (Finance Manager)

GL Entry:
Debit:  3100 Revenue   INR 10,000  -- un-earned
Credit: 1200 AR        INR 10,000  -- Tata Steel owes less
...
Net Tata Steel owes: INR 1,64,000
...
GL Entry:
Debit:  4100 Bad Debt Expense  INR 1,74,000  -- loss
Credit: 1200 AR                INR 1,74,000  -- remove from AR
...
Invoice:   INR 1,74,000
Paid:      INR 1,00,000  -- already received, stays
Write-off: INR 74,000    -- remaining balance only

GL Entry:
Debit:  4100 Bad Debt Expense  INR 74,000
Credit: 1200 AR                INR 74,000
...
Debit:  3100 Revenue      INR 1,50,000  -- reverse revenue
Debit:  2200 Tax Payable  INR   24,000  -- reverse tax liability
Credit: 1200 AR           INR 1,74,000  -- remove receivable
...
----------|-----------|-------------
Current   | #1003     | INR 80,000
30 days   | #1002     | INR 74,000
60 days   | -         | INR 0
90+ days  | -         | INR 0
----------|-----------|-------------
Total     |           | INR 1,54,000
...
Invoice detail:
#1002  due Jan 1   overdue 30 days  INR 74,000  -- call collections
#1003  due Feb 1   current          INR 80,000  -- monitor
```
...
-----------|-------|------------------|--------------------------|--------------------------------------------
Jan 15     | JE001 | Invoice Approved | 1200 AR INR 1,74,000     | 3100 Rev INR 1,50,000 + 2200 Tax Payable INR 24,000
Jan 20     | JE002 | Payment Received | 1100 Cash INR 1,00,000   | 1200 AR INR 1,00,000
Jan 25     | JE003 | Credit Memo      | 3100 Rev INR 10,000      | 1200 AR INR 10,000
-----------|-------|------------------|--------------------------|--------------------------------------------
Net AR outstanding: INR 64,000
```
...
GL Account 1200 (AR):
Sum of all journal entry lines = INR 2,34,000

AR Subledger:
Invoice #1001  Tata Steel   INR 64,000    (partially paid)
Invoice #1002  JSW Steel    INR 1,40,000  (open)
Invoice #1003  Adani Ports  INR 30,000    (open)
                            ----------
Total:                      INR 2,34,000
...
Mismatch scenario:
GL shows:        INR 2,34,000
Subledger shows: INR 2,44,000
Difference:      INR 10,000 -- ALERT! Finance team notified immediately
```
...
base_currency:        INR
base_amount:          INR 83,000  -- for CFO reporting
```

**Payment with FX Gain/Loss:**
```
Invoice date:  Jan 1   rate=83  $1,000 = INR 83,000
Payment date:  Mar 1   rate=86  $1,000 = INR 86,000
FX Gain = INR 3,000

GL Entry on payment:
Debit:  1100 Cash          INR 86,000  -- actual cash received
Credit: 1200 AR            INR 83,000  -- original invoice amount
Credit: 4300 FX Gain/Loss  INR 3,000   -- difference
```
...
----------------------|----------|-------------|----------------
Reliance Retail (IN)  | INR      | INR 1,74,000 | INR 1,74,000
Reliance USA          | USD      | $1,000       | INR 83,000
Reliance Sri Lanka    | LKR      | LKR 50,000   | INR 10,250
----------------------|----------|-------------|----------------
Total (INR)           |          |              | INR 2,67,250
...
Error: "January 2024 is LOCKED. Cannot post.
...
Debit:  AR         INR 10,00,000
Credit: Revenue    INR 10,00,000

Reliance Jio books (E003):
Debit:  IT Expense INR 10,00,000
Credit: AP         INR 10,00,000

Consolidated Report (T001):
                 Retail          Jio             Elimination      Consolidated
Revenue:         INR 10,00,000   INR 0           -INR 10,00,000   INR 0
Expense:         INR 0           INR 10,00,000   -INR 10,00,000   INR 0
AR:              INR 10,00,000   INR 0           -INR 10,00,000   INR 0
AP:              INR 0           INR 10,00,000   -INR 10,00,000   INR 0
```
...
Scenario: Reconciliation found INR 10,000 mismatch
          GL shows INR 2,44,000, Subledger shows INR 2,34,000
...
Entry:
Debit:  1200 AR        INR 10,000
Credit: 3100 Revenue   INR 10,000
