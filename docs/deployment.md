...
Expected final output begins with `Deployment successful` and lists five
running containers:
...
6. Idempotently enable pg_cron and ensure exactly one five-minute aging job.
...
9. Concurrently refresh the aging MV so deployment health is deterministic.
10. Start the internal AR application, outbox worker, and Envoy gateway.
11. Require health through Envoy and from the stub, verify the worker, and prove
    that FastAPI port 8080 is not published to the host.
12. Optionally seed data and/or run `test_api.sh`.
...
build correctly reported migrations 002, 004, and 005 as
already present, reused the single pg_cron job, refreshed the MV concurrently,
and returned all five services in a running/healthy state. Repeated executions
