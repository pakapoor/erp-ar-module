...
# With arg: creates against that invoice. No arg: uses last invoice from session.
# Usage: ./6_credit_memo.sh [INVOICE_ID] [AMOUNT] [REASON_CODE]
...
AMOUNT=${2:-1000}
REASON_CODE=${3:-OVERCHARGE}
IDEM_KEY="debug-cm-$INVOICE_ID-$(date +%s)"

echo ""
echo "=== Creating Credit Memo for Invoice: $INVOICE_ID ==="
echo "Amount:      $AMOUNT"
echo "Reason:      $REASON_CODE"
echo ""

curl -s --max-time 600 -X POST "$BASE_URL/invoices/$INVOICE_ID/credit-memos" \
  ...
  -d @- <<JSON | python3 -m json.tool
{
  ...
  "description": "Credit memo created in debug session",
  "amount": $AMOUNT,
  "include_tax": false
}
JSON
